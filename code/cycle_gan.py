...
def training_loop(dataloader_X, dataloader_Y, test_dataloader_X, test_dataloader_Y, device, opts):
    ...
    for iteration in range(1, opts.train_iters+1):
        ...
        d_Y_loss_values.append(D_Y_loss.item())
        d_X_loss_values.append(D_X_loss.item())
        g_loss_values.append(g_loss.item())
        cycle_loss_values.append(cycle_consistency_loss.item())       


    # Plot the loss graph
    iterations = range(1, opts.train_iters + 1)
    plt.plot(iterations, d_real_loss_values, 'r', label='D_real_loss')
    plt.plot(iterations, d_fake_loss_values, 'b', label='D_fake_loss')
    plt.plot(iterations, d_Y_loss_values, 'g', label='D_Y_loss')
    plt.plot(iterations, d_X_loss_values, 'm', label='D_X_loss')
    plt.plot(iterations, g_loss_values, 'y', label='G_loss')
    plt.plot(iterations, cycle_loss_values, 'c', label='Cycle_loss')
...
def create_parser():
    ...
    # Training hyper-parameters # iteration change here
